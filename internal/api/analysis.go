...
// ---- 自定义评级脚本 API（资产工具 → 评级逻辑）----

// analysisScriptGet returns the global custom rating script.
// GET /api/analysis-script
func analysisScriptGet(c *gin.Context) {
	s, err := db.GetAnalysisScript()
	if err != nil {
		log.Printf("[script] 读取失败: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "读取脚本失败"})
		return
	}
	c.JSON(http.StatusOK, s)
}

// analysisScriptPut saves the global script and hot-reloads the engine.
// PUT /api/analysis-script  body: {code, enabled}
// 保存时校验语法；启用状态/内容变化时清除当日分析缓存（缓存存有旧引擎结果）。
func analysisScriptPut(c *gin.Context) {
	var req struct {
	...
	if req.Enabled && strings.TrimSpace(req.Code) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "启用前请先填写脚本"})
		return
	}
	if req.Enabled {
		if err := market.ValidateScript(req.Code); err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "脚本语法错误：" + err.Error()})
			return
		}
	}
	prev, _ := db.GetAnalysisScript()
	now := time.Now().Format("2006-01-02 15:04:05")
	if err := db.SaveAnalysisScript(req.Code, req.Enabled, now); err != nil {
		log.Printf("[script] 保存失败: %v", err)
		c.JSON(http.StatusInternalServerError, gin.H{"error": "保存失败"})
		return
	}
	market.ConfigureEngine(req.Code, req.Enabled)
	// 引擎或脚本变化 → 旧缓存里的 engine 结论不再可信，清除当日全部缓存
	if prev == nil || prev.Code != req.Code || prev.Enabled != req.Enabled {
		if e := db.ClearAnalysisCacheByDate(time.Now().Format("2006-01-02")); e != nil {
			log.Printf("[script] 清理当日缓存失败: %v", e)
		}
	}
	c.JSON(http.StatusOK, gin.H{"ok": true, "updated_at": now})
}

// analysisScriptTest runs default vs custom logic against one holding's latest
// indicators so the editor can show a side-by-side contrast before enabling.
// POST /api/analysis-script/test  body: {code, holding_id}
// K线优先取当日缓存，未命中才实时抓取。
func analysisScriptTest(c *gin.Context) {
	var req struct {
		Code      string `json:"code"`
	...
	if strings.TrimSpace(req.Code) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "脚本为空"})
		return
	}
	if err := market.ValidateScript(req.Code); err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"error": "脚本语法错误：" + err.Error()})
	...
	var bars []market.KlineBar
	if row, e := db.GetAnalysisCache(symbol, today); e == nil && row != nil {
		_ = json.Unmarshal([]byte(row.BarsJSON), &bars)
	}
	if len(bars) < 30 {
		bars, err = market.FetchKline(symbol)
		if err != nil || len(bars) < 30 {
			c.JSON(http.StatusOK, gin.H{"error": "K线数据获取失败或不足30个交易日"})
			return
		}
	}
	...
	ind := market.CalculateIndicators(bars)
	if ind == nil {
		c.JSON(http.StatusOK, gin.H{"error": "指标计算失败"})
		return
	}
	...
	out := gin.H{
		...
		"name":      h.Name,
		"price":     ind.Price,
		"generate_at": time.Now().Format("2006-01-02 15:04:05"),
	}
	// 默认逻辑对照
	if def := market.CalculateProbability(ind); def != nil {
		out["default_up_pct"] = def.UpPct
		out["default_signal"] = market.SignalFromUpPct(def.UpPct)
	}
	// 自定义脚本结果（语法已过，这里只可能运行错误/超时）
	if up, e := market.RunScriptTest(req.Code, ind); e != nil {
		out["custom_error"] = e.Error()
	} else {
		out["custom_up_pct"] = up
		out["custom_signal"] = market.SignalFromUpPct(up)
	}
